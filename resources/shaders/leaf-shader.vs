...
void main()
{
	...
	vec4 localModelPos = localModel * vec4(l_pos, 1.0);
	v_darken = (localModelPos.y + scale / 2) / scale;
	localModelPos.y += sin(time * 2 + vec3(globalModel * vec4(0.0,0.0,0.0,1.0)).z / 3) / 5;

	gl_Position = projection * viewModel * localModelPos;
