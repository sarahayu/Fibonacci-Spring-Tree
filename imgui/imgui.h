...
#endif
#define IM_vectorSIZE(_ARR)          ((int)(sizeof(_ARR) / sizeof(*_ARR)))       // Size of a static C-style vector. Don't use on pointers!
...
namespace ImGui
{
    ...
    // - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every functions, note that a 'float v[X]' function argument is the same as 'float* v', the vector syntax is just a way to document the number of elements that are expected to be accessible. You can pass address of your first element out of a contiguous set, e.g. &myvector.x
    ...
    // - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the vector syntax is just a way to document the number of elements that are expected to be accessible.
...
// User fill ImGuiIO.KeyMap[] vector with indices into the ImGuiIO.KeysDown[512] vector
enum ImGuiKey_
{
    ImGuiKey_Tab,
    ImGuiKey_LeftArrow,
...
// Keyboard: Set io.ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard to enable. NewFrame() will automatically fill io.NavInputs[] based on your io.KeysDown[] + io.KeyMap[] vectors.
...
struct ImGuiIO
{
    ...
    int         KeyMap[ImGuiKey_COUNT];         // <unset>          // Map of indices into the KeysDown[512] entries vector which represent your "native" keyboard state.
...
struct ImGuiInputTextCallbackData
{
    ...
    int                 BufSize;        // Buffer size (in bytes) = capacity+1  // Read-only    // [Resize,Completion,History,Always] Include zero-terminator storage. In C land == vectorSIZE(my_char_vector), in C++ land: string.capacity()+1
...
// Hold a series of drawing commands. The user provides a renderer for ImDrawData which essentially contains an vector of ImDrawList.
//-----------------------------------------------------------------------------
...
struct ImDrawCmd
{
    unsigned int    ElemCount;              // Number of indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] vector, indices in idx_buffer[].
...
struct ImFontConfig
{
    ...
    const ImWchar*  GlyphRanges;            // NULL     // Pointer to a user-provided list of Unicode range (2 value per range, values are inclusive, zero-terminated list). THE vector DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE.
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline bool     GetBit(int n) const { int off = (n >> 5); ImU32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0; }  // Get bit n in the vector
    inline void     SetBit(int n)       { int off = (n >> 5); ImU32 mask = 1u << (n & 31); UsedChars[off] |= mask; }               // Set bit n in the vector
...
// Common pitfalls:
// - If you pass a 'glyph_ranges' vector to AddFont*** functions, you need to make sure that your vector persist up until the
